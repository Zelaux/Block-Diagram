...
const TextareaExtension = (function () {
    function setStyleOptions(target, preItem) {
    ...
    class SelectionInserter {
        ...
        substring(start, end, includeEnd = false) {
            let text = this.text;
            let currentCursor = this.skipCursors(start);
            if (currentCursor === undefined || currentCursor >= end && !includeEnd)
                return text.substring(start, end);
            if (start == currentCursor)
                return cursorElement + text.substring(start, end);
            return text.substring(start, currentCursor) + cursorElement + text.substring(currentCursor, end);
        }
        skipCursors(start) {
            let currentCursor = this.cursorsList[this.cursorI];
