...
(function (Tools) {
    class TokenWithPosition {
        constructor(token, inTokenPosition) {
            this.newStart = 0;
            this.token = token;
            this.inTokenPosition = inTokenPosition;
        }
        position(len) {
            if (this === TokenWithPosition.END)
                return len;
            return this.newStart + this.inTokenPosition;
        }
    }
    TokenWithPosition.END = new TokenWithPosition(null, 0);
    function isSkipable(token) {
        return token.kind == TokenKind.WhiteSpace || token.kind == TokenKind.NL;
    }
    function autoformatInArea(target) {
        ...
        function findTokenPosition(position, stickyForward) {
            let i = Lexer.getTokenAtChar(tokens, position, !stickyForward);
            if (i == -1) {
                if (position == text.length)
                    return TokenWithPosition.END;
                i = tokens.length - 1;
        ...
        let newText = "";
        let wasNL = false;
        function addContent(content, token) {
        ...
        target.setSelectionRange(cursors[0].position(newText.length), cursors[1].position(newText.length), "forward");
    }
    Tools.autoformatInArea = autoformatInArea;
})(Tools || (Tools = {}));
